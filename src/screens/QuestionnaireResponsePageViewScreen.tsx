...
export default function QuestionnaireResponsePageViewScreen({route, navigation}) {
  const { page, questionnaireResponse } = route.params;
      ...
      <View className="p-8 flex-1">
        <View className='flex-1'>
          ...
          <ScrollView className='mt-2 flex-1'>
            {<QRAnswers questions={page.questions.filter((question) => !question.questionGroup)}></QRAnswers>}
            {Object.entries(page.questions.filter((question) => question.questionGroup).reduce((groups, question) => {
                ...
                return (<QRAnswerGroup key={groupedQuestions.groupName} questions={groupedQuestions.questions}>{groupedQuestions.groupName}</QRAnswerGroup>)
              })
            }
          </ScrollView>
        </View>
